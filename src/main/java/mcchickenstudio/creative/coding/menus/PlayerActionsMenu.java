...
package mcchickenstudio.creative.coding.menus;
 ...
 public class PlayerActionsMenu extends LegacyMenu {
    ...
    public PlayerActionsMenu(Player player, int page, List<PlayerActionSubtype> actionList, Location signLoc) {

        super(6,getLocaleMessage("blocks.action_player",false));

        int[] eventsSlots = PlayerActionsMenu.actionSlots;
        ...
        PlayerActionsMenu.PlayerActionCategory[] actionCategories = PlayerActionsMenu.PlayerActionCategory.values();
        for(int slotCategory = 0; slotCategory < categoriesSlots.length-2; slotCategory++) {
            PlayerActionsMenu.PlayerActionCategory actionCategory = actionCategories[slotCategory];
        ...
        if (actionList.isEmpty()) {
            ...
            List<List<PlayerActionSubtype>> allPages = getPagesForActions(actionList);
            int pageToOpen = page;
            ...
            int slot = 0;
            for (PlayerActionSubtype action : allPages.get(pageToOpen-1)) {
    ...
    public static void openInventory(Player player, int page, Location signLoc) {
        player.openInventory(new PlayerActionsMenu(player,page,PlayerActionCategory.COMMUNICATION.getActionsByCategory(),signLoc).getInventory());
    }

    public static void openInventory(Player player, int page, List<PlayerActionSubtype> actionList, Location signLoc) {
        player.openInventory(new PlayerActionsMenu(player,page,actionList,signLoc).getInventory());
    }

     public static void openInventory(Player player, int page, PlayerActionCategory category, Location signLoc) {
         player.openInventory(new PlayerActionsMenu(player,page,category.getActionsByCategory(),signLoc).getInventory());
     }

    public static ItemStack getNoActionsButton() {
        ItemStack item = new ItemStack(Material.BARRIER);
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(getLocaleItemName("items.developer.categories.actions.no-actions.name"));
        meta.setLore(getLocaleItemDescription("items.developer.categories.actions.no-actions.lore"));
    ...
    private static List<List<PlayerActionSubtype>> getPagesForActions(List<PlayerActionSubtype> plotsList) {

        List<List<PlayerActionSubtype>> pages = new ArrayList<>();
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            List<PlayerActionSubtype> sublist = plotsList.subList(fromIndex, toIndex);
            ArrayList<PlayerActionSubtype> page = new ArrayList<>(sublist);
     ...
     public enum PlayerActionCategory {
         ...
         COMMUNICATION("communication",Material.BOOK),
         ...
         PlayerActionCategory(String messagePath, Material material) {
             this.messagePath = messagePath;
             this.material = material;
         }

         public String getMessagePath() {
             return "items.developer.categories.actions." + messagePath;
         }
         ...
         public List<PlayerActionSubtype> getActionsByCategory() {
             List<PlayerActionSubtype> subtypes = new ArrayList<>();
             for (PlayerActionSubtype subtype : PlayerActionSubtype.values()) {
                 if (subtype.getActionCategory() == this) {
     ...
     public static PlayerActionCategory getCategoryClicked(Material material) {

         for (PlayerActionCategory actionCategory : PlayerActionCategory.values()) {
